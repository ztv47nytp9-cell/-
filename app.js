...
function captureRegisterDraft(){
  ...
  registerFormDraft={mode:registerMode,flow:registerFlow,type:value("recType"),warehouse:value("recWarehouse"),targetWarehouse:value("recTargetWarehouse"),date:value("recDate"),title:value("recTitle"),locationType:value("recLocationType"),locationDetail:value("recLocationDetail"),memo:value("recMemo"),items:draftItems.map(item=>({...item})),equipmentItems:draftEquipmentItems.map(item=>({...item})),savedAt:new Date().toISOString()};
...
function recordRow(r){
  ...
  const statusText = r.status === "pending" ? "진행중" : r.type;
  const count=(r.items||[]).length+(r.equipmentItems||[]).length;
  const place=r.location?.detail || r.warehouse || "보관 미지정";
  return `<button class="list-row" data-detail="${r.id}" type="button">
    <div>
      <div><span class="badge ${cls}">${esc(statusText)}</span>${count?` <span class="mini-chip">${count}건</span>`:""}</div>
      <div class="row-title" style="margin-top:7px">${esc(r.title)}</div>
      <div class="row-sub">${fmtDate(r.date)} · ${esc(place)} · ${esc(summarizeItems(r.items,r.equipmentItems,r.flow))}</div>
    </div>
    <div class="chev">›</div>
  </button>`;
}
...
function resourceOverview(){
  const materialKinds = catalog.filter(item=>warehouses.some(w=>Number(state.stock[w]?.[item.name]||0)>0)).length;
  const equipmentCount = (state.equipment||[]).reduce((sum,item)=>sum+Number(item.qty||0),0);
  const emptyWarehouses = warehouses.filter(name=>{
    const summary=warehouseSummary(name);
    return summary.materialCount===0 && summary.equipmentCount===0;
  }).length;
  return {warehouses:warehouses.length,materialKinds,equipmentCount,emptyWarehouses};
}

function storageSummaryCard(clickable=false){
  const s = totalStorageSummary();
  const o = resourceOverview();
  const tag = clickable ? "button" : "div";
  return `<${tag} class="summary-card${clickable ? " summary-link" : ""}" ${clickable ? 'id="homeStorageSummary" type="button"' : ""}>
    <div class="section-head mini-head"><div class="section-title">전체 보관량</div>${clickable?`<span class="mini-link">전체 보기 ›</span>`:""}</div>
    ...
    <div class="summary-subline">보관 ${o.warehouses}곳 · 보유 자재 ${o.materialKinds}종 · 장비 ${o.equipmentCount}대${o.emptyWarehouses?` · 빈 곳 ${o.emptyWarehouses}`:""}</div>
  </${tag}>`;
}
...
function equipmentTransferSnapshot(equipment){
  return equipment ? {
    cat:equipment.cat || "기타장비", name:equipment.name, spec:equipment.spec || equipment.detail || "", detail:equipment.detail || equipment.spec || "",
    model:equipment.model || "", memo:equipment.memo || equipment.etc || "", etc:equipment.etc || equipment.memo || "",
    battery:equipment.battery || "", fuel:equipment.fuel || "", status:equipment.status || "정상"
  } : null;
}

function matchingEquipmentAtPlace(snapshot,place,excludeId=""){
  if(!snapshot) return null;
  return (state.equipment || []).find(item=>item.id!==excludeId&&item.place===place&&item.name===snapshot.name&&String(item.spec||item.detail||"")===String(snapshot.spec||snapshot.detail||"")&&String(item.model||"")===String(snapshot.model||""));
}

function applyEquipmentTransfers(equipmentItems,fromWarehouse,toWarehouse){
  return (equipmentItems || []).map(entry=>{
    const equipment=state.equipment.find(item=>item.id===entry.id);
    if(!equipment) return entry;
    const currentQty=Math.max(0,Math.round(Number(equipment.qty || 0)));
    const qty=Math.min(currentQty,Math.max(1,Math.round(Number(entry.qty || 0))));
    const snapshot=equipmentTransferSnapshot(equipment);
    const moved={...entry,qty,fromPlace:equipment.place || fromWarehouse,toPlace:toWarehouse,sourceSnapshot:snapshot,beforeQty:currentQty};
    equipment.moves=equipment.moves || [];
    if(qty>=currentQty){
      equipment.moves.push({id:uid(),date:todayISO(),from:equipment.place || fromWarehouse,to:toWarehouse,memo:"이송 등록",createdAt:new Date().toISOString()});
      equipment.place=toWarehouse;
      equipment.updatedAt=new Date().toISOString();
      return {...moved,transferMode:"full",toEquipmentId:equipment.id,afterQty:0,targetBefore:0,targetAfter:qty};
    }
    equipment.qty=currentQty-qty;
    equipment.updatedAt=new Date().toISOString();
    equipment.moves.push({id:uid(),date:todayISO(),from:equipment.place || fromWarehouse,to:toWarehouse,memo:`부분 이송 ${qty}대`,createdAt:new Date().toISOString()});
    let target=matchingEquipmentAtPlace(snapshot,toWarehouse,equipment.id);
    const targetBefore=Number(target?.qty || 0);
    if(target){
      target.qty=targetBefore+qty;
      target.updatedAt=new Date().toISOString();
    }else{
      target={id:uid(),...snapshot,qty,place:toWarehouse,photo:"",photos:[],maintenance:[],moves:[{id:uid(),date:todayISO(),from:equipment.place || fromWarehouse,to:toWarehouse,memo:`부분 이송 ${qty}대`,createdAt:new Date().toISOString()}],updatedAt:new Date().toISOString()};
      state.equipment.push(target);
    }
    return {...moved,transferMode:"partial",toEquipmentId:target.id,afterQty:equipment.qty,targetBefore,targetAfter:targetBefore+qty};
  });
}

function restoreEquipmentTransfer(entry){
  const qty=Math.max(0,Math.round(Number(entry.qty || 0)));
  const fromPlace=entry.fromPlace || entry.place || "";
  const toPlace=entry.toPlace || "";
  const snapshot=entry.sourceSnapshot || {cat:entry.cat || "기타장비",name:entry.name,spec:entry.spec || "",detail:entry.spec || "",model:entry.model || ""};
  if(entry.transferMode==="full"){
    const moved=state.equipment.find(item=>item.id===(entry.toEquipmentId || entry.id));
    if(moved){
      moved.place=fromPlace;
      moved.qty=Number(entry.beforeQty || moved.qty || qty || 1);
      moved.updatedAt=new Date().toISOString();
    }
    return;
  }
  const target=state.equipment.find(item=>item.id===entry.toEquipmentId);
  if(target){
    target.qty=Math.max(0,Number(target.qty || 0)-qty);
    target.updatedAt=new Date().toISOString();
    if(target.qty<=0) state.equipment=state.equipment.filter(item=>item.id!==target.id);
  }
  let source=state.equipment.find(item=>item.id===entry.id) || matchingEquipmentAtPlace(snapshot,fromPlace,entry.toEquipmentId);
  if(source){
    source.qty=Number(source.qty || 0)+qty;
    source.place=fromPlace;
    source.updatedAt=new Date().toISOString();
  }else{
    state.equipment.push({id:entry.id || uid(),...snapshot,qty,place:fromPlace,photo:"",photos:[],maintenance:[],moves:[],updatedAt:new Date().toISOString()});
  }
}
  ...
  view.innerHTML = `
    ...
    <div class="home-quick card">
      ...
      <div class="home-quick-grid">
        <button class="quick-action emergency" id="homeQuickEmergency" type="button"><span class="urgent-mark">🚨</span><strong>긴급기록</strong><small>사용량·메모</small></button>
        <button class="quick-action" id="homeQuickOut" type="button"><span>↗</span><strong>출고</strong><small>재고 차감</small></button>
        <button class="quick-action" id="homeQuickIn" type="button"><span>↙</span><strong>입고</strong><small>재고 증가</small></button>
      </div>
    </div>
    <div class="card">
    ...
    <div class="activity-tabs"><button class="${homeActivityTab === "history" ? "active" : ""}" id="homeActivityHistory" type="button">이력 ${recentCount}</button><button class="${homeActivityTab === "memo" ? "active" : ""}" id="homeActivityMemo" type="button">메모 ${memoCount}</button></div>
...
function renderAllMaterialList(){
  ...
  const totalQty=rows.reduce((sum,item)=>sum+Number(item.total||0),0);
  const zeroCount=rows.filter(item=>Number(item.total||0)===0).length;
  document.getElementById("allMaterialCount").textContent = `검색 결과 ${rows.length}종 · 총 ${Number(totalQty).toLocaleString("ko-KR")} · 0재고 ${zeroCount}종`;
...
function renderAllEquipmentList(){
  ...
  const equipmentQtyTotal=rows.reduce((sum,item)=>sum+Number(item.qty||0),0);
  const equipmentPlaceCount=new Set(rows.map(item=>item.place||"보관 미지정")).size;
  document.getElementById("allEquipmentCount").textContent = `검색 결과 ${rows.length}건 · 총 ${equipmentQtyTotal}대 · ${equipmentPlaceCount}곳`;
...
function renderHnsView(){
  return `<div class="card">
    ...
    <div class="emptybox compact-empty-action">HNS 자료는 정식 자료를 받은 뒤 별도로 구성합니다.<br>검수 전 물질별 대응정보는 표시하지 않습니다.<button class="btn secondary compact" id="hnsTechniqueLink" type="button">방제기술 보기</button></div>
  </div>`;
}
...
function renderWarehouse(){
  if(!selectedWarehouse){
    ...
    if(warehouseViewMode === "materials"){
      ...
      document.getElementById("hnsTechniqueLink")?.addEventListener("click",()=>setPage("technique"));
    }else{
  ...
  const whSummary = warehouseSummary(selectedWarehouse);
  view.innerHTML = warehouseViewSwitcher() + warehouseActions(selectedWarehouse) + `
    <button class="back" id="backWh" type="button">‹ 보관</button>
    <div class="card"><div class="section-head mini-head"><div class="section-title">${esc(selectedWarehouse)}</div><span class="mini-chip">${esc(warehouseKind(selectedWarehouse))}</span></div><div class="warehouse-mini-stats"><span>자재 ${whSummary.materialCount}종</span><span>장비 ${whSummary.equipmentCount}종</span></div><div class="row-sub">📌 중요 메모</div><div class="memo-box">${info.memo ? esc(info.memo) : "등록된 메모가 없습니다."}</div>${warehouseFieldMemoHtml(info)}<button class="btn secondary" id="editInfo" type="button" style="width:100%;margin-top:12px">메모 수정</button></div>
  ...
  document.getElementById("addFuelSupply")?.addEventListener("click",()=>openFuelSupplyForm(selectedWarehouse));
  document.getElementById("editFuelBase")?.addEventListener("click",()=>openFuelStockEditForm(selectedWarehouse));
...
function quickRecordCard(){
  return `<div class="callout compact-draft">사용 내역과 메모만 남기고, 재고 반영은 나중에 합니다.</div>`;
}

function registerDraftSummaryHtml(){
  const materialRows=draftItems.filter(item=>Number(item.qty)>0).length;
  const equipmentRows=draftEquipmentItems.filter(item=>Number(item.qty)>0).length;
  const totalQty=draftItems.reduce((sum,item)=>sum+Number(item.qty||0),0);
  if(!draftItems.length&&!draftEquipmentItems.length) return "";
...
function transferTargetOptions(from=""){
  const list=warehouses.filter(name=>name!==from);
  return (list.length?list:warehouses).map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join("");
}

function syncTransferTarget(){
  const from=document.getElementById("recWarehouse")?.value || "";
  const target=document.getElementById("recTargetWarehouse");
  if(!target) return;
  const previous=target.value;
  target.innerHTML=transferTargetOptions(from);
  if(previous&&previous!==from&&warehouses.includes(previous)) target.value=previous;
  else target.value=warehouses.find(name=>name!==from) || "";
}
...
function renderRegister(){
  ...
  view.innerHTML = `
    ...
    <div class="form">
      ...
      `}
      ${registerMode === "normal" && registerFlow !== "이송" ? `<div class="field-card" id="registerLocationCard">
        <div class="section-title" style="font-size:16px">사고 위치</div>
      ...
      <div id="registerDraftSummary">${registerDraftSummaryHtml()}</div>
      <button class="btn primary sticky-action" id="saveRecord" type="button">${registerMode === "quick" ? "진행 중 사건 저장" : registerFlow + " 저장"}</button>
    </div>
  `;
  ...
  document.getElementById("recWarehouse")?.addEventListener("change",()=>{syncTransferTarget();renderItems();renderEquipmentItems();updateStockAfterPreview();});
  ...
  if(registerFormDraft){[["recType","type"],["recWarehouse","warehouse"],["recDate","date"],["recTitle","title"],["recMemo","memo"]].forEach(([id,key])=>{const input=document.getElementById(id);if(input&&registerFormDraft[key]!==undefined)input.value=registerFormDraft[key];});syncTransferTarget();const targetInput=document.getElementById("recTargetWarehouse");if(targetInput&&registerFormDraft.targetWarehouse&&registerFormDraft.targetWarehouse!==document.getElementById("recWarehouse")?.value)targetInput.value=registerFormDraft.targetWarehouse;renderItems();renderEquipmentItems();updateStockAfterPreview();}
...
function updateRegisterTypeDependentUI(){
  const card=document.getElementById("registerLocationCard");
  const isAccident=document.getElementById("recType")?.value === "사고";
  if(card){
    card.style.display=isAccident?"":"none";
    if(!isAccident){
      const detail=document.getElementById("recLocationDetail");
      if(detail) detail.value="";
    }
  }
  const saveButton=document.getElementById("saveRecord");
  if(saveButton&&registerMode==="normal"){
    saveButton.textContent=isAccident&&registerFlow!=="이송"?"진행 중 사건 저장":registerFlow+" 저장";
  }
}
...
function collectRegisterLocation(){
  if(document.getElementById("recType")?.value !== "사고") return null;
...
function openRegisterMaterialPicker(){
  ...
  const renderNames=()=>{
    ...
    if(select)select.innerHTML=rows.map(item=>`<option value="${esc(item.name)}">${esc(materialLocationLabel(item,warehouse))}</option>`).join("");
    refreshResourceLocations();
  };
...
function equipmentRegisterLabel(equipment){
  ...
  return `[${equipment.place || "보관 미지정"}] ${equipment.name} · ${spec}${model}`;
}
...
function renderEquipmentItems(){
  ...
  area.innerHTML=`<div class="group-title">${registerMode==="normal"&&registerFlow==="입고"?"입고 장비":registerMode==="normal"&&registerFlow==="이송"?"이송 장비":"사용 장비"}</div>`+draftEquipmentItems.map((item,index) => {
    const selected=selectableEquipment.find(e=>e.id===item.id) || state.equipment.find(e=>e.id===item.id) || selectableEquipment[0] || state.equipment[0];
    const selectedCategory=selected?.cat || item.cat || "기타장비";
    const equipmentSource=selectableEquipment;
    const categories=[...new Set([...equipmentSource.map(e=>e.cat || "기타장비"),selectedCategory])].sort((a,b)=>a.localeCompare(b,"ko"));
    const categoryEquipment=equipmentSource.filter(e=>(e.cat || "기타장비")===selectedCategory).sort((a,b)=>equipmentRegisterLabel(a).localeCompare(equipmentRegisterLabel(b),"ko"));
    if(selected&&!categoryEquipment.some(e=>e.id===selected.id))categoryEquipment.unshift(selected);
    return `<div class="item-box"><div class="form">
      ...
      <label>${registerMode==="normal"&&registerFlow==="입고"?"입고":registerMode==="normal"&&registerFlow==="이송"?"이송":"사용"} 수량 (등록 ${Number(selected?.qty || 0)}대)<input type="number" inputmode="numeric" min="1" step="1" data-equipment-qty="${index}" value="${item.qty ?? ""}"></label>
  ...
  area.querySelectorAll("[data-equipment-category]").forEach(select=>select.addEventListener("change",()=>{
    const index=Number(select.dataset.equipmentCategory);const equipment=(selectableEquipment.length?selectableEquipment:(state.equipment || [])).filter(e=>(e.cat || "기타장비")===select.value).sort((a,b)=>equipmentRegisterLabel(a).localeCompare(equipmentRegisterLabel(b),"ko"))[0];if(!equipment)return;
...
async function saveRecord(){
  ...
  const warehouse = document.getElementById("recWarehouse").value;
  const targetWarehouse = document.getElementById("recTargetWarehouse")?.value || "";
  ...
  if(registerMode === "normal" && type === "사고" && registerFlow !== "이송"){
    if(saveButton){saveButton.disabled=true;saveButton.textContent="저장 중";}
    state.records.push(createFlowRecord({
  ...
  if(["출고","이송"].includes(registerFlow) && !stockAvailable(warehouse, items)){
  ...
  if(registerFlow === "이송"){
    ...
    const movedEquipmentItems=applyEquipmentTransfers(equipmentItems,warehouse,targetWarehouse);
    state.records.push(createFlowRecord({
...
function summarizeItems(items,equipmentItems=[],flow=""){ 
  ...
  (equipmentItems || []).slice(0,Math.max(0,2-parts.length)).forEach(item=>parts.push(`${item.name} ${item.qty}대 ${flow==="입고"?"반납":flow==="이송"?"이송":"사용"}`));
...
function renderHistory(){
  ...
  view.innerHTML = `
    <div class="history-tabs">
      ...
      <button class="history-tab ${historyFilter === "pending" ? "active" : ""}" data-hfilter="pending" type="button">진행중 ${counts.pending}</button>
...
function renderHistoryListHtml(records){
  if(!records.length) return `<div class="emptybox compact-empty-action">기록이 없습니다.<button class="btn secondary compact" id="emptyHistoryRegister" type="button">등록하기</button></div>`;
...
function bindHistoryRows(){
  ...
  document.getElementById("emptyHistoryRegister")?.addEventListener("click",()=>startHomeRegister("normal","출고"));
}

function recordFieldSummary(r){
  const location=r.location?.detail?`<div class="detail-row"><div class="detail-label">사고 위치</div><div class="detail-value">${esc(r.location.type||"위치")} · ${esc(r.location.detail)}</div></div>`:"";
  return location?`<div class="detail-grid">${location}</div>`:"";
}

function openRecordBasicEdit(id){
  const r=state.records.find(item=>item.id===id);
  if(!r || r.status==="pending") return;
  const location=r.location||{};
  openEntryModal(`${entryHeader("이력 기본정보 수정","수량과 재고는 바꾸지 않습니다")}
    <div class="form">
  ...
  document.getElementById("saveRecordBasicEdit")?.addEventListener("click",()=>{
    ...
    r.editHistory.push({id:uid(),editedAt:new Date().toISOString(),before});
...
async function savePendingEdits(id, shouldApply=false, silent=false){
  ...
  if(changedItems.length) r.items = changedItems;
...
async function addPendingEquipmentToRecord(id){
  const r = state.records.find(x => x.id === id);
  if(!r || r.status !== "pending") return;
  if(!await savePendingEdits(id,false,true)) return;
  const used=new Set((r.equipmentItems||[]).map(item=>item.id));
  const equipment=state.equipment.find(item=>!used.has(item.id)) || state.equipment[0];
  if(!equipment){showFeedback("info","추가할 장비가 없습니다");return;}
  r.equipmentItems=r.equipmentItems||[];
  r.equipmentItems.push({id:equipment.id,name:equipment.name,qty:1,place:equipment.place,spec:equipment.spec || equipment.detail || ""});
  save();
  openDetail(id,{push:false,remember:false});
}

async function removePendingEquipmentFromRecord(id,index){
  const r = state.records.find(x => x.id === id);
  if(!r || r.status !== "pending") return;
  if(!await savePendingEdits(id,false,true)) return;
  r.equipmentItems=(r.equipmentItems||[]).filter((_,i)=>i!==index);
  save();
  openDetail(id,{push:false,remember:false});
}
...
function openDetail(id, options={}){
  ...
  if(r.status === "pending"){
    view.innerHTML = `
      ...
      ${pendingRecordForm(r)}
      <button class="btn danger detail-delete-btn" id="deleteRecord" type="button">진행 중 사건 삭제</button>`;
    refreshResourceLocations();
    ...
    view.querySelectorAll("[data-remove-pending-equipment]").forEach(button => button.addEventListener("click", () => removePendingEquipmentFromRecord(id, Number(button.dataset.removePendingEquipment))));
    document.getElementById("deleteRecord")?.addEventListener("click", async () => {
      if(!await askConfirm("진행 중 사건 삭제","재고에는 영향이 없습니다.","삭제",true)) return;
  ...
  document.getElementById("deleteRecord")?.addEventListener("click", async () => {
    if(!await askConfirm("기록 삭제","선택한 기록을 삭제할까요?","삭제",true)) return;
    const undoState=JSON.stringify(state);
    state.trash=state.trash||[];state.trash.push({id:uid(),kind:"record",data:JSON.parse(JSON.stringify(r)),deletedAt:new Date().toISOString()});
    if(r.flow === "재고수정"){
...
function openFastSurveySetup(){
  ...
  openEntryModal(`${entryHeader("초기 재고실사","실사하지 않은 품목은 기존 수량을 유지합니다")}<div class="form"><label>창고<select id="fastSurveyWarehouse"><option value="all">전체 창고</option>${warehouses.map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join("")}</select></label><label>분류<select id="fastSurveyCategory"><option value="all">전체 분류</option>${catsNow.map(cat=>`<option>${esc(cat)}</option>`).join("")}</select></label><button class="btn primary" id="startFastSurvey" type="button">실사 시작</button></div>`);
  document.getElementById("startFastSurvey")?.addEventListener("click",()=>{
    const scope={warehouse:document.getElementById("fastSurveyWarehouse").value,category:document.getElementById("fastSurveyCategory").value};
    const items=buildFastSurveyItems(scope),beforeState=JSON.stringify(state);
    state.survey={active:true,index:0,startedAt:new Date().toISOString(),values:{},checked:{},scope,items,baseline:Object.fromEntries(items.map(item=>[`${item.warehouse}|${item.name}`,Number(state.stock[item.warehouse]?.[item.name]||0)])),lastApplied:state.survey?.lastApplied||null};
    if(!saveStateTransaction(beforeState,"실사 시작 정보를 저장하지 못했습니다. 기존 자료는 유지했습니다"))return;
    closeEntryModal();renderFastSurvey();
  });
}
function buildFastSurveyItems(scope={warehouse:"all",category:"all"}){return warehouses.filter(w=>scope.warehouse==="all"||w===scope.warehouse).flatMap(w=>catalog.filter(i=>scope.category==="all"||i.cat===scope.category).map(i=>({warehouse:w,name:i.name,cat:i.cat,unit:i.unit})));}
function ensureFastSurveyState(){
  const survey=state.survey;
  if(!survey)return null;
  survey.scope=survey.scope||{warehouse:"all",category:"all"};
  survey.values=survey.values&&typeof survey.values==="object"?survey.values:{};
  survey.checked=survey.checked&&typeof survey.checked==="object"?survey.checked:Object.fromEntries(Object.keys(survey.values).map(key=>[key,true]));
  if(!Array.isArray(survey.items))survey.items=buildFastSurveyItems(survey.scope);
  if(!survey.baseline||typeof survey.baseline!=="object")survey.baseline=Object.fromEntries(survey.items.map(item=>[`${item.warehouse}|${item.name}`,Number(state.stock[item.warehouse]?.[item.name]||0)]));
  return survey;
}
function fastSurveyItems(){return ensureFastSurveyState()?.items||[];}
function fastSurveyConfirmedCount(items=fastSurveyItems()){return items.reduce((count,item)=>count+(state.survey?.checked?.[`${item.warehouse}|${item.name}`]===true?1:0),0);}
function fastSurveyConflicts(items=fastSurveyItems()){
  return items.filter(item=>{
    const key=`${item.warehouse}|${item.name}`,stillExists=warehouses.includes(item.warehouse)&&catalog.some(row=>row.name===item.name);
    return !stillExists||Number(state.stock[item.warehouse]?.[item.name]||0)!==Number(state.survey?.baseline?.[key]||0);
  });
}
async function restartFastSurvey(){if(!await askConfirm("재고실사 다시 시작","현재 실사 입력을 지우고 새 기준 수량으로 다시 시작할까요?","다시 시작",true))return;const beforeState=JSON.stringify(state);state.survey={active:false,index:0,values:{},checked:{},items:[],baseline:{},lastApplied:state.survey?.lastApplied||null};if(!saveStateTransaction(beforeState,"실사 초기화를 저장하지 못했습니다"))return;openFastSurveySetup();}
function renderFastSurvey(){
  const survey=ensureFastSurveyState(),items=fastSurveyItems(),idx=Math.min(survey.index||0,Math.max(0,items.length-1)),it=items[idx];page="survey";
  if(!it){showFeedback("info","선택 범위에 품목이 없습니다");setPage("home");return;}
  const key=`${it.warehouse}|${it.name}`,cur=Number(state.stock[it.warehouse]?.[it.name]||0),baseline=Number(survey.baseline[key]||0),checked=survey.checked[key]===true,confirmed=fastSurveyConfirmedCount(items),saved=survey.values[key];
  document.getElementById("headTitle").innerHTML=`<div class="page-title">초기 재고실사</div><div class="date">확인 ${confirmed} / ${items.length} · ${idx+1}번째</div>`;
  view.innerHTML=`<button class="back" id="exitFastSurvey" type="button">‹ 홈</button><div class="card survey-card"><span class="badge blue">${esc(it.warehouse)}</span><div class="survey-item-status ${checked?"confirmed":"unconfirmed"}">${checked?"✓ 확인 완료":"미확인"}</div><div class="section-title" style="margin-top:10px">${esc(it.name)}</div><div class="row-sub">${esc(it.cat)} · 실사 시작 수량 ${qtyText(baseline,it.unit)}${cur!==baseline?` · 현재 수량 ${qtyText(cur,it.unit)} (변경됨)`:""}</div><label>실사 수량<input class="big-input" id="fastSurveyQty" type="number" inputmode="decimal" min="0" value="${checked?saved??"":""}" placeholder="세어 본 수량 입력"></label><div class="survey-fast-actions"><button class="btn gray" id="surveyZero" type="button">실제 수량 0</button><button class="btn secondary" id="surveyKeepCurrent" type="button">현재 수량 확인</button><button class="btn primary" id="surveyDirect" type="button">수량 직접 입력</button></div></div><div class="entry-actions"><button class="btn gray" id="fastPrev" type="button" ${idx===0?"disabled":""}>이전</button><button class="btn primary" id="fastNext" type="button">${idx===items.length-1?"확인 현황 보기":"다음 · 건너뛰기"}</button></div><div class="row-sub survey-progress-note">확인 ${confirmed}개 · 미확인 ${items.length-confirmed}개</div><button class="btn secondary" id="fastBatch" type="button" style="width:100%;margin-top:9px">여러 품목 한 번에 입력</button><button class="btn secondary" id="fastReview" type="button" style="width:100%;margin-top:9px">확인 현황 보기</button>`;
  const commit=async(value)=>{const beforeState=JSON.stringify(state);survey.values[key]=Number(value);survey.checked[key]=true;if(!saveStateTransaction(beforeState,"실사 수량을 저장하지 못했습니다. 다시 저장해주세요"))return;const next=Math.min(idx+1,items.length-1);if(next===idx)renderFastSurveyReview();else{const beforeMove=JSON.stringify(state);survey.index=next;if(saveStateTransaction(beforeMove,"실사 진행 위치를 저장하지 못했습니다"))renderFastSurvey();}};
  document.getElementById("surveyZero").onclick=()=>commit(0);document.getElementById("surveyKeepCurrent").onclick=()=>commit(cur);document.getElementById("surveyDirect").onclick=()=>document.getElementById("fastSurveyQty").focus();
  document.getElementById("fastNext").onclick=async()=>{const input=document.getElementById("fastSurveyQty");if(input.value!==""){commit(input.value);return;}const beforeState=JSON.stringify(state);survey.checked[key]=false;delete survey.values[key];if(idx===items.length-1){if(saveStateTransaction(beforeState,"실사 진행 정보를 저장하지 못했습니다"))renderFastSurveyReview();return;}survey.index=idx+1;if(saveStateTransaction(beforeState,"실사 진행 정보를 저장하지 못했습니다"))renderFastSurvey();};
  document.getElementById("fastPrev").onclick=()=>{const beforeState=JSON.stringify(state);survey.index=Math.max(0,idx-1);if(saveStateTransaction(beforeState,"실사 진행 위치를 저장하지 못했습니다"))renderFastSurvey();};
  document.getElementById("fastBatch").onclick=renderFastSurveyBatch;document.getElementById("fastReview").onclick=renderFastSurveyReview;document.getElementById("exitFastSurvey").onclick=()=>setPage("home");
}
function renderFastSurveyBatch(){
  const survey=ensureFastSurveyState(),items=fastSurveyItems(),start=Math.floor((survey.index||0)/12)*12,rows=items.slice(start,start+12);
  view.innerHTML=`<button class="back" id="batchBack" type="button">‹ 한 품목씩</button><div class="card"><div class="section-title">일괄 재고입력</div><div class="row-sub">빈칸은 미확인으로 남고, 0도 확인 수량으로 처리합니다.</div>${rows.map((it,n)=>{const key=`${it.warehouse}|${it.name}`,checked=survey.checked[key]===true;return `<div class="batch-row"><div><strong>${esc(it.name)}</strong><div class="row-sub">${esc(it.warehouse)} · 시작 수량 ${qtyText(survey.baseline[key]||0,it.unit)} · ${checked?"확인 완료":"미확인"}</div><button class="link-btn" type="button" data-batch-current="${n}">현재 수량 확인</button></div><input data-batch="${n}" type="number" inputmode="decimal" min="0" value="${checked?survey.values[key]??"":""}" placeholder="미확인"></div>`;}).join("")}</div><button class="btn primary" id="batchSave" type="button" style="width:100%">확인 저장 · 다음 묶음</button>`;
  document.getElementById("batchBack").onclick=renderFastSurvey;
  document.querySelectorAll("[data-batch-current]").forEach(button=>button.onclick=()=>{const n=Number(button.dataset.batchCurrent),item=rows[n],input=document.querySelector(`[data-batch="${n}"]`);input.value=Number(state.stock[item.warehouse]?.[item.name]||0);});
  document.getElementById("batchSave").onclick=()=>{const beforeState=JSON.stringify(state);rows.forEach((it,n)=>{const key=`${it.warehouse}|${it.name}`,input=document.querySelector(`[data-batch="${n}"]`);if(input.value!==""){survey.values[key]=Number(input.value);survey.checked[key]=true;}else{delete survey.values[key];survey.checked[key]=false;}});survey.index=Math.min(start+12,items.length-1);if(!saveStateTransaction(beforeState,"일괄 실사 입력을 저장하지 못했습니다. 입력을 확인하고 다시 시도해주세요"))return;start+12>=items.length?renderFastSurveyReview():renderFastSurveyBatch();};
}
function renderSurveySummary(summary){
  ...
  view.innerHTML=`<div class="card"><div class="section-title">재고실사 완료 요약</div><div class="summary-grid survey-summary-grid"><div class="summary-pill"><div class="summary-label">확인 품목</div><div class="summary-value">${Number(summary.checked||0).toLocaleString("ko-KR")}건</div></div><div class="summary-pill"><div class="summary-label">미확인</div><div class="summary-value">${Number(summary.unchecked||0).toLocaleString("ko-KR")}건</div></div><div class="summary-pill"><div class="summary-label">0재고</div><div class="summary-value">${Number(summary.zeroCount||0).toLocaleString("ko-KR")}건</div></div><div class="summary-pill"><div class="summary-label">변동</div><div class="summary-value">${Number(summary.changedCount||0).toLocaleString("ko-KR")}건</div></div></div>${summary.memo?`<div class="callout" style="margin-top:12px">${esc(summary.memo)}</div>`:""}</div><div class="card"><div class="section-title">다음 작업</div><div class="row-sub">필요하면 이력에서 재고수정 기록을 확인하고, 문제가 있으면 점3개 메뉴의 실사 반영 취소를 사용할 수 있습니다.</div></div><button class="btn primary" id="surveySummaryHome" type="button" style="width:100%">홈으로</button>`;
  document.getElementById("surveySummaryHome")?.addEventListener("click",()=>setPage("home"));
}

function renderFastSurveyReview(){
  const survey=ensureFastSurveyState(),items=fastSurveyItems(),confirmed=items.filter(it=>survey.checked[`${it.warehouse}|${it.name}`]===true),unchecked=items.filter(it=>survey.checked[`${it.warehouse}|${it.name}`]!==true),conflicts=fastSurveyConflicts(items);
  const reviewed=confirmed.map(it=>{const key=`${it.warehouse}|${it.name}`;return {...it,before:Number(survey.baseline[key]||0),after:Number(survey.values[key]||0)};});
  const changed=reviewed.filter(it=>it.before!==it.after),zeroCount=reviewed.filter(it=>Number(it.after||0)===0).length,unchangedCount=reviewed.length-changed.length;
  page="survey";document.getElementById("headTitle").innerHTML=`<div class="page-title">실사 확인</div><div class="date">확인 ${confirmed.length} / ${items.length}</div>`;
  view.innerHTML=`<button class="back" id="reviewBack" type="button">‹ 실사 입력</button><div class="card"><div class="section-title">실사 요약</div><div class="summary-grid"><div class="summary-pill"><div class="summary-label">확인</div><div class="summary-value">${confirmed.length}개</div></div><div class="summary-pill"><div class="summary-label">미확인</div><div class="summary-value">${unchecked.length}개</div></div><div class="summary-pill"><div class="summary-label">수량 변경</div><div class="summary-value">${changed.length}개</div></div></div><div class="row-sub" style="margin-top:10px">확인 중 0재고 ${zeroCount}개 · 수량 변동 없음 ${unchangedCount}개</div></div>${conflicts.length?`<div class="callout error-callout"><strong>재고 기준이 바뀐 항목 ${conflicts.length}개</strong><div>실사 중 다른 작업에서 수량이나 품목 구성이 바뀌었습니다. 잘못 덮어쓰지 않도록 반영을 막았습니다. 새 기준으로 실사를 다시 시작해주세요.</div><button class="btn secondary" id="restartConflictedSurvey" type="button">새 기준으로 다시 시작</button></div>`:""}<div class="list-card"><div class="section-title">확인한 수량 변경 ${changed.length}건</div>${changed.length?changed.map(it=>`<div class="list-row"><div><div class="row-title">${esc(it.name)}</div><div class="row-sub">${esc(it.warehouse)} · ${qtyText(it.before,it.unit)} → ${qtyText(it.after,it.unit)}</div></div></div>`).join(""):`<div class="emptybox">확인한 수량 변경이 없습니다.</div>`}</div>${unchecked.length?`<label class="survey-skip-option"><input id="allowUncheckedApply" type="checkbox"> 미확인 ${unchecked.length}개는 기존 수량을 유지하고, 확인한 품목만 반영</label>`:""}<div class="card"><label>담당 메모<textarea id="surveyApplyMemo" placeholder="실사 특이사항 또는 담당 메모"></textarea></label></div><button class="btn primary" id="applyFastSurvey" type="button" style="width:100%" ${conflicts.length||unchecked.length?"disabled":""}>확인한 품목 반영</button>`;
  document.getElementById("reviewBack").onclick=renderFastSurvey;
  document.getElementById("restartConflictedSurvey")?.addEventListener("click",restartFastSurvey);
  const applyButton=document.getElementById("applyFastSurvey"),skipOption=document.getElementById("allowUncheckedApply");
  skipOption?.addEventListener("change",()=>{applyButton.disabled=!skipOption.checked||conflicts.length>0;});
  applyButton.onclick=async()=>{
    if(conflicts.length){showFeedback("error","기준 재고가 바뀌어 반영할 수 없습니다");return;}
    if(unchecked.length&&!skipOption?.checked){showFeedback("info","미확인 품목을 유지할지 선택해주세요");return;}
    if(!await askConfirm("재고실사 반영",`${confirmed.length}개 확인 수량을 반영하고 미확인 ${unchecked.length}개는 기존 수량으로 유지할까요?`,"반영"))return;
    const beforeState=JSON.stringify(state),summary={date:todayISO(),checked:confirmed.length,unchecked:unchecked.length,zeroCount,changedCount:changed.length,unchangedCount,memo:document.getElementById("surveyApplyMemo")?.value.trim()||""};
    const lastApplied={appliedAt:new Date().toISOString(),changes:changed.map(it=>({warehouse:it.warehouse,name:it.name,before:it.before,after:it.after})),summary};
    confirmed.forEach(it=>{if(!state.stock[it.warehouse])state.stock[it.warehouse]={};state.stock[it.warehouse][it.name]=Number(survey.values[`${it.warehouse}|${it.name}`]);});
    state.survey={active:false,index:0,values:{},checked:{},items:[],baseline:{},done:true,endedAt:new Date().toISOString(),lastApplied,lastSummary:summary};
    if(!saveStateTransaction(beforeState,"실사 결과를 저장하지 못했습니다. 입력과 기존 재고를 복원했습니다"))return;
    showFeedback("success",`재고실사 반영 · 확인 ${confirmed.length}개 · 변경 ${changed.length}개`);renderSurveySummary(summary);
  };
}

async function undoLastSurvey(){const applied=state.survey?.lastApplied;if(!applied?.changes?.length)return;if(!await askConfirm("실사 반영 취소",`${applied.changes.length}개 품목을 실사 전 수량으로 되돌릴까요?`,"되돌리기",true))return;const beforeState=JSON.stringify(state);applied.changes.forEach(change=>{if(state.stock[change.warehouse])state.stock[change.warehouse][change.name]=Number(change.before||0);});state.survey.lastApplied=null;if(!saveStateTransaction(beforeState,"실사 취소를 저장하지 못했습니다. 현재 재고를 유지했습니다"))return;showFeedback("success","직전 실사 반영을 취소했습니다");renderHome();}
...
function vesselFuelTotals(place){const now=todayISO(),month=now.slice(0,7),year=now.slice(0,4),logs=state.assetOps?.[place]?.logs||[];return {current:Number(state.assetOps?.[place]?.fuelBase||0),month:logs.filter(log=>String(log.date||"").slice(0,7)===month).reduce((sum,log)=>sum+Number(log.fuel||0),0),year:logs.filter(log=>String(log.date||"").slice(0,4)===year).reduce((sum,log)=>sum+Number(log.fuel||0),0)};}
function vesselFuelRows(place){const rows=[...(state.assetOps?.[place]?.fuelLogs||[])].slice(-5).reverse();return rows.length?rows.map(log=>`<div class="row-sub">${fmtDate(log.date)} · ${Number(log.amount||0)>0?"+":""}${Number(log.amount||0)}L${log.memo?` · ${esc(log.memo)}`:""}</div>`).join(""):`<div class="row-sub">연료 수급 이력이 없습니다.</div>`;}
function vesselMonthlySummary(place,month=todayISO().slice(0,7)){const logs=(state.assetOps?.[place]?.logs||[]).filter(log=>String(log.date||"").slice(0,7)===month);return {logs,count:logs.length,mileage:logs.reduce((sum,log)=>sum+Number(log.mileageDiff||0),0),port:logs.reduce((sum,log)=>sum+Number(log.portDiff||0),0),starboard:logs.reduce((sum,log)=>sum+Number(log.starboardDiff||0),0)};}
...
function renderOps(place){
  ...
  if(isVehiclePlace(place)){
    return `<div class="card"><div class="section-title">차량 관리</div>
      ...
      <button class="btn gray" id="openVesselHistory" type="button" style="width:100%;margin-top:9px">전체 이력 보기</button>
  ...
  const fuel=vesselFuelTotals(place);
  return `<div class="card"><div class="section-title">${esc(place)} 관리</div>
    <div class="grid2">
      ...
      <div class="metric" style="height:82px"><div><div class="metric-label">연료유 재고</div><div class="metric-value">${fuel.current}L</div><div class="row-sub">월 ${fuel.month}L · 년 ${fuel.year}L 소모</div></div></div>
    </div>
    <div class="callout"><strong>이번 달 운항 요약</strong><br>기록 ${monthly.count}건 · ${monthly.mileage}NM · 좌현 ${monthly.port}h · 우현 ${monthly.starboard}h · 연료 ${fuel.month}L</div>
    <div class="btn-row" style="display:grid;grid-template-columns:1fr 1fr;margin-top:12px">
    ...
    <div class="btn-row" style="display:grid;grid-template-columns:1fr 1fr;margin-top:9px">
        <button class="btn secondary" id="addFuelSupply" type="button">연료 수급</button>
        <button class="btn gray" id="editFuelBase" type="button">연료 재고수정</button>
    </div>
    <div class="callout" style="margin-top:12px"><strong>최근 연료 수급</strong><br>${vesselFuelRows(place)}</div>
    <button class="btn secondary" id="addAssetMaintenance" type="button" style="width:100%;margin-top:9px">+ 정비이력 추가</button>
    <div class="section-title" style="margin-top:16px">최근 이력</div>${recentOpRows(place)}
    <button class="btn gray" id="openVesselHistory" type="button" style="width:100%;margin-top:9px">전체 이력 보기</button>
    <div class="section-title" style="margin-top:16px">정비이력</div>${assetMaintenanceRows(place)}
...
function openFuelSupplyForm(place){
  const op=state.assetOps?.[place];if(!op)return;op.fuelLogs=op.fuelLogs||[];
  openEntryModal(`${entryHeader("연료 수급",place)}<div class="form"><label>날짜<input id="fuelSupplyDate" type="date" value="${todayISO()}"></label><label>수급량(L)<input id="fuelSupplyAmount" type="number" inputmode="decimal" min="0" step="0.1" placeholder="예: 200"></label><label>메모<textarea id="fuelSupplyMemo" placeholder="수급처나 특이사항"></textarea></label><button class="btn primary" id="saveFuelSupply" type="button">저장</button></div>`);
  document.getElementById("saveFuelSupply")?.addEventListener("click",()=>{const amount=Number(document.getElementById("fuelSupplyAmount").value||0);if(!Number.isFinite(amount)||amount<=0){showFeedback("error","수급량을 입력해주세요");return;}op.fuelBase=Number(op.fuelBase||0)+amount;op.fuelLogs.push({id:uid(),date:document.getElementById("fuelSupplyDate").value||todayISO(),amount,memo:document.getElementById("fuelSupplyMemo").value.trim(),createdAt:new Date().toISOString()});save();closeEntryModal();showFeedback("success",`연료 ${amount}L 수급 저장`);renderWarehouse();});
}

function openFuelStockEditForm(place){
  const op=state.assetOps?.[place];if(!op)return;op.fuelLogs=op.fuelLogs||[];
  openEntryModal(`${entryHeader("연료 재고수정",place)}<div class="form"><div class="callout">현재 기록 재고 ${Number(op.fuelBase||0)}L</div><label>현재 연료유 재고(L)<input id="fuelStockValue" type="number" inputmode="decimal" min="0" step="0.1" value="${Number(op.fuelBase||0)}"></label><label>메모<textarea id="fuelStockMemo" placeholder="수정 사유"></textarea></label><button class="btn primary" id="saveFuelStock" type="button">덮어쓰기 저장</button></div>`);
  document.getElementById("saveFuelStock")?.addEventListener("click",()=>{const before=Number(op.fuelBase||0),after=Number(document.getElementById("fuelStockValue").value);if(!Number.isFinite(after)||after<0){showFeedback("error","연료 재고량을 확인해주세요");return;}op.fuelBase=after;op.fuelLogs.push({id:uid(),date:todayISO(),amount:after-before,memo:document.getElementById("fuelStockMemo").value.trim()||"재고수정",before,after,createdAt:new Date().toISOString()});save();closeEntryModal();showFeedback("success",`연료 재고 ${after}L 저장`);renderWarehouse();});
}
...
function recentOpRows(place){
  const logs = [...((state.assetOps?.[place]?.logs) || [])].sort((a,b)=>historyDateKey(b).localeCompare(historyDateKey(a))).slice(0,10);
...
function filterAssetHistory(rows,{from="",to="",kind="",query=""}={}){
  ...
  return rows.filter(row=>{
    const date=String(row.log.date || row.log.createdAt || "").slice(0,10);
    return (!from||date>=from)&&(!to||date<=to)&&(!kind||row.kind===kind)&&terms.every(term=>JSON.stringify(row.log).toLocaleLowerCase().includes(term)||row.kind.includes(term));
  });
}
function assetHistoryRowHtml({kind,log}){
  const labels={date:"일자",createdAt:"입력 시각",updatedAt:"수정 시각",time:"시간",type:"처리유형",flow:"입출고",title:"제목",content:"내용",departure:"출발 위치",arrival:"도착 위치",from:"출발 보관",to:"도착 보관",warehouse:"보관장소",targetWarehouse:"도착 보관",location:"작업 위치",memo:"메모",parts:"부속품",cost:"비용(원)",before:"이전 누적값",after:"현재 누적값",diff:"증감",distance:"주행거리(km)",hours:"엔진시간(h)",mileageBefore:"이전 마일(NM)",mileageAfter:"현재 마일(NM)",mileageDiff:"운항거리(NM)",portBefore:"이전 좌현(h)",portAfter:"현재 좌현(h)",portDiff:"좌현 사용(h)",starboardBefore:"이전 우현(h)",starboardAfter:"현재 우현(h)",starboardDiff:"우현 사용(h)",fuel:"연료 소모(L)",amount:"연료 증감(L)",operator:"운항자",passengers:"동승자",area:"활동해역",status:"상태"};
  const valueText=(value)=>Array.isArray(value)?value.map(valueText).join("\n"):value&&typeof value==="object"?Object.entries(value).filter(([key])=>!["id","photo","photos"].includes(key)).map(([key,val])=>`${labels[key]||({name:"이름",qty:"수량",unit:"단위",place:"보관장소",cat:"분류",spec:"규격",kind:"구분",detail:"상세",model:"모델"}[key])||key}: ${valueText(val)}`).join(" · "):String(value??"");
  const title=log.title || log.content || kind+" 기록";
  const route=[log.departure||log.from||log.warehouse,log.arrival||log.to||log.targetWarehouse].filter(Boolean).join(" → ");
  const fields=Object.entries(log).filter(([key,value])=>!["id","items","equipmentItems","title","memo","date","photo","photos"].includes(key)&&value!==null&&value!==undefined&&value!=="");
  const resources=[...(log.items||[]),...(log.equipmentItems||[])];
const metrics=[];
  const metric=(label,value,unit)=>{if(value!==undefined&&value!==null&&value!=="")metrics.push(label+" "+value+unit);};
  if(kind==="주행")metric("주행",log.diff??log.distance,"km");
  if(kind==="운항"){
    metric("운항",log.mileageDiff,"NM");metric("좌현",log.portDiff,"h");metric("우현",log.starboardDiff,"h");
    if(log.portDiff===undefined)metric("엔진",log.hours??log.diff,"h");
    metric("연료 소모",log.fuel,"L");
  }
  if(kind==="연료")metric("연료 증감",log.amount,"L");
  return `<article class="history-box asset-history-row"><span class="badge blue">${esc(kind)}</span><div class="row-title">${esc(title)}</div><div class="row-sub">${esc(log.date||"날짜 미지정")}${route?" · "+esc(route):""}</div>${metrics.length?`<div class="row-sub">${esc(metrics.join(" · "))}</div>`:""}${log.memo?`<div class="history-memo">${esc(log.memo)}</div>`:""}${resources.map(item=>`<div class="row-sub">${esc(item.name||"이름 미지정")} · ${esc(item.qty??"")} ${esc(item.unit||"대")}${item.place?" · "+esc(item.place):""}</div>`).join("")}<details><summary>기록 상세</summary><dl class="history-fields">${fields.map(([key,value])=>`<div><dt>${esc(labels[key]||key)}</dt><dd>${esc(valueText(value))}</dd></div>`).join("")}${resources.length?`<div><dt>자원 상세</dt><dd>${esc(valueText(resources))}</dd></div>`:""}</dl></details></article>`;
}
function openAssetHistory(place,equipmentId=""){
  const equipment=equipmentId?state.equipment.find(item=>item.id===equipmentId):null;
  const rows=assetHistoryEntries(place,equipmentId);
  const kinds=[...new Set(rows.map(row=>row.kind))];
  openEntryModal(`${entryHeader("전체 이력",equipment?.name||place)}<div class="form"><label>검색<input id="assetHistoryQuery" type="search" placeholder="메모 · 위치 · 자재 · 장비"></label><details><summary>기간·처리유형</summary><div class="history-filter-grid"><label>시작일<input id="assetHistoryFrom" type="date"></label><label>종료일<input id="assetHistoryTo" type="date"></label></div><label>처리유형<select id="assetHistoryKind"><option value="">전체</option>${kinds.map(kind=>`<option>${esc(kind)}</option>`).join("")}</select></label></details></div><div id="assetHistorySummary" class="row-sub" role="status" aria-live="polite"></div><div id="assetHistoryList"></div><button class="btn secondary" id="assetHistoryMore" type="button" style="width:100%">이력 더보기</button>`);
  let filtered=rows,shown=0,timer;
  const list=document.getElementById("assetHistoryList"),more=document.getElementById("assetHistoryMore"),summary=document.getElementById("assetHistorySummary");
  const append=()=>{
    const next=filtered.slice(shown,shown+20);
    list.insertAdjacentHTML("beforeend",next.map(assetHistoryRowHtml).join(""));
    shown+=next.length;more.hidden=shown>=filtered.length;
    summary.textContent=`검색 결과 ${filtered.length}건 · ${shown}건 표시`;
  };
  const update=()=>{
    if(!list.isConnected)return;
    const from=document.getElementById("assetHistoryFrom").value,to=document.getElementById("assetHistoryTo").value;
    filtered=filterAssetHistory(rows,{from,to,kind:document.getElementById("assetHistoryKind").value,query:document.getElementById("assetHistoryQuery").value});
    shown=0;list.innerHTML="";append();
    if(from&&to&&from>to){summary.textContent="시작일과 종료일을 확인해주세요";return;}
    if(!filtered.length)list.innerHTML='<div class="emptybox">조건에 맞는 이력이 없습니다.</div>';
  };
  more.addEventListener("click",append);
  ["assetHistoryFrom","assetHistoryTo","assetHistoryKind"].forEach(id=>document.getElementById(id).addEventListener("change",update));
  document.getElementById("assetHistoryQuery").addEventListener("input",()=>{clearTimeout(timer);timer=setTimeout(update,150);});
  update();
}
function openVesselHistory(place){openAssetHistory(place);}
...
function openEquipment(id, options={}){
  ...
  const historyLimit=Math.max(10,Number(options.historyLimit)||10);
  view.innerHTML = `
    ...
    <div class="card resource-detail-card">
      ...
      <div class="section-title" style="margin-top:10px">${esc(item.name)}</div><button class="btn secondary" id="openEquipmentHistory" type="button">전체 이력 보기</button>
    ...
    <div class="card"><div class="section-head" style="margin:0 0 10px"><div class="section-title">관리이력</div><button class="btn secondary compact" id="addMaintenance" type="button">+ 이력 추가</button></div>
      ${maintenance.length ? maintenance.slice(0,historyLimit).map(log=>`<div class="history-box"><div><span class="badge orange">${esc(log.type)}</span></div><div class="row-title" style="margin-top:7px">${esc(log.content || "관리 기록")}</div><div class="row-sub">${fmtDate(log.date)}${log.parts?.length ? ` · ${esc(log.parts.map(part=>`${part.name} ${part.qty}개`).join(", "))}` : ""}</div>${log.memo?`<div class="row-sub">${esc(log.memo)}</div>`:""}<div class="btn-row" style="margin-top:9px"><button class="btn gray compact" data-maint-edit="${log.id}" type="button">수정</button><button class="btn danger compact" data-maint-delete="${log.id}" type="button">삭제</button></div></div>`).join("") : `<div class="emptybox">등록된 관리이력이 없습니다.</div>`}
    </div>
    <div class="card"><div class="section-title">사용이력</div>${usage.length ? usage.slice(0,historyLimit).map(record=>{const entry=record.equipmentItems.find(e=>e.id===item.id);return `<button class="stock-line" data-usage-record="${record.id}" type="button"><div><div class="stock-name">${esc(record.title)}</div><div class="stock-spec">${fmtDate(record.date)} · ${esc(record.type)}</div></div><div class="stock-qty">${Number(entry.qty)}대</div></button>`;}).join("") : `<div class="emptybox">등록된 사용이력이 없습니다.</div>`}</div>
    <div class="card"><div class="section-title">이동이력</div>${moves.length ? moves.slice(0,historyLimit).map(move=>`<div class="stock-line"><div><div class="stock-name">${esc(move.from)} → ${esc(move.to)}</div><div class="stock-spec">${fmtDate(move.date)}${move.memo?` · ${esc(move.memo)}`:""}</div></div></div>`).join("") : `<div class="emptybox">등록된 이동이력이 없습니다.</div>`}</div>`;
  if(Math.max(maintenance.length,usage.length,moves.length)>historyLimit){
    const more=document.createElement("button");more.type="button";more.className="btn secondary";more.textContent="이력 더보기";more.style.width="100%";
    more.addEventListener("click",()=>openEquipment(item.id,{push:false,remember:false,historyLimit:historyLimit+20}));
    view.append(more);
  }
...
function closeEntryModal(){
  if(document.activeElement?.closest?.("#entryModal"))document.activeElement.blur();
...
function addMaterialChoice(){ openExistingMaterialAdd(); }
function addEquipmentChoice(){ openExistingEquipmentAdd(); }
...
function openExistingMaterialAdd(){
  ...
  const firstCat=sortedCats[0] || "";
  openEntryModal(`${entryHeader("자재 추가",place)}
    <div class="form">
      <div class="global-search-hint">기존 목록에서 먼저 고르고, 목록에 없으면 하단 신규추가를 누르세요.</div>
      ...
      <button class="btn secondary" id="openNewMaterialFromExisting" type="button">+ 신규 자재 추가</button>
    </div>`);
  const renderNames=()=>{
  ...
  document.getElementById("openNewMaterialFromExisting")?.addEventListener("click",()=>simpleResourceForm("material"));
  renderNames();
...
function openExistingEquipmentAdd(){
  ...
  openEntryModal(`${entryHeader("장비 추가",place)}
    <div class="form">
      <div class="global-search-hint">기존 목록에서 먼저 고르고, 목록에 없으면 하단 신규추가를 누르세요.</div>
      ...
      <button class="btn secondary" id="openNewEquipmentFromExisting" type="button">+ 신규 장비 추가</button>
    </div>`);
  const renderNames=()=>{
  ...
  document.getElementById("openNewEquipmentFromExisting")?.addEventListener("click",()=>simpleResourceForm("equipment"));
  renderNames();
...
function openMaterialTransferForm(){
  const from=selectedWarehouse || warehouses[0] || "";
  const target=warehouses.find(name=>name!==from) || "";
  applyRegisterDraft({mode:"normal",flow:"이송",type:"이송",warehouse:from,targetWarehouse:target,date:todayISO(),title:"창고 간 이동",memo:"",items:[],equipmentItems:[]});
  setPage("register");
}
...
function cloudDashboardHtml(latest=null,{loading=false,error=""}={}){
  ...
  const lastAction=meta.lastAppliedAt&&(!meta.lastUploadedAt||dateMs(meta.lastAppliedAt)>=dateMs(meta.lastUploadedAt))?`방금 가져옴 ${fmtShareTime(meta.lastAppliedAt)}`:meta.lastUploadedAt?`마지막 올림 ${fmtShareTime(meta.lastUploadedAt)}`:"공유 기록 없음";
  return `${entryHeader("자원 공유","공유자료 상태를 확인합니다")}
    <div class="form">
      <div class="share-status-card ${latest?"ready":""}">
        ...
        <div class="share-time-note">${esc(lastAction)}</div>
      </div>
...
function updateInputViewport(){
  ...
  root.style.setProperty("--keyboard-space",focused?Math.max(0,window.innerHeight-height-top)+"px":"0px");
}
function ensureInputVisible(){
  const input=document.activeElement;
  if(!input?.matches?.("input,textarea,select"))return;
  const viewport=window.visualViewport;
  const top=(viewport?.offsetTop||0)+12,bottom=(viewport?.offsetTop||0)+(viewport?.height||window.innerHeight)-16;
  const dialog=input.closest(".dialog");
  const bounds=input.getBoundingClientRect();
  const containerBounds=dialog?.getBoundingClientRect();
  const visibleTop=Math.max(top,containerBounds?containerBounds.top+12:(document.querySelector(".top")?.getBoundingClientRect().bottom||0)+8);
  const visibleBottom=Math.min(bottom,containerBounds?containerBounds.bottom-12:bottom);
  const delta=bounds.bottom>visibleBottom?Math.min(bounds.bottom-visibleBottom,bounds.top-visibleTop):bounds.top<visibleTop?bounds.top-visibleTop:0;
  if(Math.abs(delta)>1){
...
function scheduleInputVisibility(){
  cancelAnimationFrame(inputViewportFrame);
  inputViewportFrame=requestAnimationFrame(()=>{updateInputViewport();ensureInputVisible();});
}
function bindInputViewport(){
  updateInputViewport();
  window.visualViewport?.addEventListener("resize",scheduleInputVisibility);
  window.visualViewport?.addEventListener("scroll",updateInputViewport);
  window.addEventListener("resize",scheduleInputVisibility);
  document.addEventListener("focusin",()=>{scheduleInputVisibility();setTimeout(scheduleInputVisibility,300);});
  document.addEventListener("input",event=>{if(event.target?.matches?.("textarea"))scheduleInputVisibility();});
  document.addEventListener("change",event=>{
    if(event.target?.matches?.("#recWarehouse,#pendingWarehouse,select[data-name],select[data-equipment-name],select[id^=pendingName],select[id^=pendingEquipment],#registerMaterialName,#registerEquipmentName"))refreshResourceLocations();
  });
}
function materialLocationLabel(item,warehouse){
  return `${warehouse||"보관 미지정"} · ${item.name} · ${Number(state.stock[warehouse]?.[item.name]||0)}${item.unit||""}`;
}
function refreshResourceLocations(){
  scheduleInputVisibility();
  document.querySelectorAll('select[data-name],select[data-equipment-name],select[id^="pendingName"],select[id^="pendingEquipment"],#registerMaterialName,#registerEquipmentName').forEach(select=>{
    const equipment=select.matches('select[data-equipment-name],select[id^="pendingEquipment"],#registerEquipmentName');
    const item=equipment?state.equipment.find(row=>row.id===select.value):itemOf(select.value);
    if(!item)return;
    const warehouse=equipment?item.place:(document.getElementById(select.id.startsWith("pending")?"pendingWarehouse":"recWarehouse")?.value||"");
    if(!equipment)Array.from(select.options).forEach(option=>{
      const material=itemOf(option.value);
      if(material)option.textContent=materialLocationLabel(material,warehouse);
    });
    const text=equipment?`${item.name} · 저장위치: ${warehouse||"미지정"} · 등록 ${Number(item.qty??0)}대`:`${item.name} · ${warehouse?"저장위치: "+warehouse+" · 현재 "+Number(state.stock[warehouse]?.[item.name]||0)+(item.unit||""):"저장위치 미지정"}`;
    const label=select.closest("label");
    if(!label)return;
    let hint=label.querySelector(".resource-location");
    if(!hint){hint=document.createElement("span");hint.className="resource-location";label.append(hint);}
    hint.textContent=text;
    if(equipment&&registerMode==="normal"&&registerFlow==="이송"&&select.matches("[data-equipment-name]")&&warehouse!==document.getElementById("recWarehouse")?.value){
      hint.textContent+=" · 출발 보관의 장비로 다시 선택해주세요";
    }
  });
}
...
function bindGlobal(){
  ...
  document.addEventListener("focusout",event=>{
    if(event.target?.matches?.("input, textarea, select"))setTimeout(()=>{
...
function preventLockedBackgroundScroll(event){
  ...
  while(nested&&nested!==container){
    if(nested.scrollHeight>nested.clientHeight+1 && /auto|scroll/.test(getComputedStyle(nested).overflowY)){
      if((deltaY>0&&nested.scrollTop>0)||(deltaY<0&&nested.scrollTop+nested.clientHeight<nested.scrollHeight-1))return;
    }
...
function restoreNavigation(nav){
  ...
  try{
    ...
    if(page === "register"){
      ...
      registerFlow = ["출고","입고","이송"].includes(nav.flow) ? nav.flow : "출고";
    }
    updateBottomNav();
    render();
  }finally{
