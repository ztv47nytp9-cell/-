...
function editWarehouseInfo(w){
  ...
  document.getElementById("saveWarehouseMemo")?.addEventListener("click", () => {
    const kind=document.getElementById("warehouseKindEdit").value;state.warehouseKinds=state.warehouseKinds||{};state.warehouseKinds[w]=kind;if(kind==="차량"&&!state.assetOps[w])state.assetOps[w]={distanceBase:0,logs:[],counterMode:"absolute"};if(kind==="함정"&&!state.assetOps[w])state.assetOps[w]={hoursBase:0,portHoursBase:0,starboardHoursBase:0,engineSplitMode:"dual",fuelBase:0,logs:[],counterMode:"absolute"};
...
function renderRegister(){
  ...
  document.getElementById("recWarehouse")?.addEventListener("change",updateStockAfterPreview);
  renderItems();
...
function editOpBase(place){
  ...
  document.getElementById("saveOpBase")?.addEventListener("click",()=>{
    ...
    else{ const port=Number(document.getElementById("opBasePortHours").value),starboard=Number(document.getElementById("opBaseStarboardHours").value);if(!Number.isFinite(port)||port<0||!Number.isFinite(starboard)||starboard<0){showFeedback("error","좌·우 엔진 시간을 확인해주세요");return;}state.assetOps[place].portHoursBase=port;state.assetOps[place].starboardHoursBase=starboard;state.assetOps[place].engineSplitMode="dual"; }
...
function addOpLog(place){
  const vehicle=isVehiclePlace(place);
  openEntryModal(`${entryHeader(vehicle?"주행 기록":"운항 기록",place)}<div class="form"><label>날짜<input id="opLogDate" type="date" value="${todayISO()}"></label>${vehicle?`<label>계기판 누적거리(km)<input id="opLogDistance" type="number" inputmode="decimal" min="0" value="" placeholder="현재 계기판 값을 입력"></label>`:`<div class="callout">기록값: 좌현 ${opTotal(place,"portHours")}h · 우현 ${opTotal(place,"starboardHours")}h</div><label>좌현 엔진 누적시간(h)<input id="opLogPortHours" type="number" inputmode="decimal" min="0" value="" placeholder="좌현 계기판 값"></label><label>우현 엔진 누적시간(h)<input id="opLogStarboardHours" type="number" inputmode="decimal" min="0" value="" placeholder="우현 계기판 값"></label><label>이번 연료소모량(L)<input id="opLogFuel" type="number" inputmode="decimal" min="0" value="" placeholder="연료량"></label>`}<label>메모<textarea id="opLogMemo"></textarea></label><button class="btn primary" id="saveOpLog" type="button">저장</button></div>`);
  document.getElementById("saveOpLog")?.addEventListener("click",()=>{
    ...
    else{const portInput=document.getElementById("opLogPortHours"),starboardInput=document.getElementById("opLogStarboardHours"),portBefore=opTotal(place,"portHours"),starboardBefore=opTotal(place,"starboardHours"),portAfter=Number(portInput.value),starboardAfter=Number(starboardInput.value),fuel=Number(document.getElementById("opLogFuel").value||0);if(portInput.value===""||starboardInput.value===""||!Number.isFinite(portAfter)||portAfter<portBefore||!Number.isFinite(starboardAfter)||starboardAfter<starboardBefore||!Number.isFinite(fuel)||fuel<0){showFeedback("error","좌·우 엔진 누적시간과 연료량을 확인해주세요");return;}state.assetOps[place].portHoursBase=portAfter;state.assetOps[place].starboardHoursBase=starboardAfter;state.assetOps[place].engineSplitMode="dual";state.assetOps[place].logs.push({...common,portBefore,portAfter,portDiff:portAfter-portBefore,starboardBefore,starboardAfter,starboardDiff:starboardAfter-starboardBefore,fuel});}
...
function recentOpRows(place){
  ...
  return logs.map(l=>{const dual=l.portAfter!==undefined&&l.starboardAfter!==undefined,editable=l.after!==undefined||dual;const text=dual?`좌현 ${Number(l.portBefore||0)} → ${Number(l.portAfter)} · 우현 ${Number(l.starboardBefore||0)} → ${Number(l.starboardAfter)}`:l.after!==undefined?`${Number(l.before||0)} → ${Number(l.after)} · +${Number(l.diff||0)}${isVehiclePlace(place)?"km":"h"}`:(isVehiclePlace(place)?`기존 이력 +${Number(l.distance||0)}km`:`기존 단일시간 이력 +${Number(l.hours||0)}h`);return `<div class="list-row"><div><div class="row-title">${esc(l.date||"")}</div><div class="row-sub">${text}${isVesselPlace(place)&&Number(l.fuel||0)>0?` · 연료 ${Number(l.fuel)}L`:""}${l.memo?` · ${esc(l.memo)}`:""}</div>${l.id===latestId&&editable?`<div class="btn-row" style="margin-top:7px"><button class="btn gray compact" data-op-edit="${l.id}" type="button">수정</button><button class="btn danger compact" data-op-delete="${l.id}" type="button">삭제</button></div>`:""}</div></div>`;}).join("");
}
...
function bindOpLogActions(place){
  ...
  view.querySelector("[data-op-delete]")?.addEventListener("click",async event=>{const op=state.assetOps?.[place],log=op?.logs?.length?op.logs[op.logs.length-1]:null;if(!log||log.id!==event.currentTarget.dataset.opDelete)return;if(!await askConfirm("최근 이력 삭제","최근 기록을 삭제하고 누적값을 이전 값으로 되돌릴까요?","삭제",true))return;if(isVehiclePlace(place))op.distanceBase=Number(log.before||0);else{op.portHoursBase=Number(log.portBefore??log.before??op.portHoursBase??0);op.starboardHoursBase=Number(log.starboardBefore??log.before??op.starboardHoursBase??0);}op.logs.pop();save();showFeedback("success","최근 이력 삭제 완료");renderWarehouse();});
}
...
function editLatestOpLog(place,logId){
  ...
  const portBefore=Number(log.portBefore??log.before??0),starboardBefore=Number(log.starboardBefore??log.before??0),portAfter=Number(log.portAfter??log.after??op.portHoursBase??0),starboardAfter=Number(log.starboardAfter??log.after??op.starboardHoursBase??0);
  openEntryModal(`${entryHeader(vehicle?"최근 주행기록 수정":"최근 운항기록 수정",place)}<div class="form"><label>날짜<input id="editOpDate" type="date" value="${esc(log.date||todayISO())}"></label>${vehicle?`<div class="callout">이전 누적값 ${Number(log.before||0)}km</div><label>현재 누적거리(km)<input id="editOpAfter" type="number" inputmode="decimal" min="${Number(log.before||0)}" value="${Number(log.after||0)}"></label>`:`<div class="callout">이전값: 좌현 ${portBefore}h · 우현 ${starboardBefore}h</div><label>좌현 엔진 누적시간(h)<input id="editOpPortAfter" type="number" inputmode="decimal" min="${portBefore}" value="${portAfter}"></label><label>우현 엔진 누적시간(h)<input id="editOpStarboardAfter" type="number" inputmode="decimal" min="${starboardBefore}" value="${starboardAfter}"></label><label>연료소모량(L)<input id="editOpFuel" type="number" inputmode="decimal" min="0" value="${Number(log.fuel||0)}"></label>`}<label>메모<textarea id="editOpMemo">${esc(log.memo||"")}</textarea></label><button class="btn primary" id="saveOpEdit" type="button">수정 저장</button></div>`);
  document.getElementById("saveOpEdit")?.addEventListener("click",()=>{log.date=document.getElementById("editOpDate").value||todayISO();log.memo=document.getElementById("editOpMemo").value.trim();if(vehicle){const before=Number(log.before||0),after=Number(document.getElementById("editOpAfter").value);if(!Number.isFinite(after)||after<before){showFeedback("error","누적거리를 확인해주세요");return;}log.after=after;log.diff=after-before;op.distanceBase=after;}else{const nextPort=Number(document.getElementById("editOpPortAfter").value),nextStarboard=Number(document.getElementById("editOpStarboardAfter").value),fuel=Number(document.getElementById("editOpFuel").value||0);if(!Number.isFinite(nextPort)||nextPort<portBefore||!Number.isFinite(nextStarboard)||nextStarboard<starboardBefore||!Number.isFinite(fuel)||fuel<0){showFeedback("error","좌·우 엔진 시간과 연료량을 확인해주세요");return;}Object.assign(log,{portBefore,portAfter:nextPort,portDiff:nextPort-portBefore,starboardBefore,starboardAfter:nextStarboard,starboardDiff:nextStarboard-starboardBefore,fuel});delete log.before;delete log.after;delete log.diff;op.portHoursBase=nextPort;op.starboardHoursBase=nextStarboard;op.engineSplitMode="dual";}save();closeEntryModal();showFeedback("success","최근 이력 수정 완료");renderWarehouse();});
}
...
function addWarehouse(){
  ...
  document.getElementById("saveNewWarehouse")?.addEventListener("click", () => {
    ...
    ensureWarehouse(name);state.warehouseKinds=state.warehouseKinds||{};state.warehouseKinds[name]=kind;if(kind==="차량")state.assetOps[name]={distanceBase:0,logs:[],counterMode:"absolute"};if(kind==="함정")state.assetOps[name]={hoursBase:0,portHoursBase:0,starboardHoursBase:0,engineSplitMode:"dual",fuelBase:0,logs:[],counterMode:"absolute"};save();closeEntryModal();showFeedback("success",`${kind} 추가 완료`);if(page==="warehouse") renderWarehouse();
  });
}
...
function init(){
  ...
  if("serviceWorker" in navigator){
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("./sw.js?v=0190m")
